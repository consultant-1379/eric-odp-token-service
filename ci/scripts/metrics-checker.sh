...
checkServiceYAML(){
    SERVICE_NAME=$1
    if grep -q "{{- include \"$SERVICE_NAME.annotations\" . | nindent [0-9] }}" ./charts/$SERVICE_NAME/templates/headless-service.yaml &&
...
checkCoreApplicationJAVA(){
    ...
    find ./odp-token-service-api/src/main/java -regex '.*Application.java' -exec grep -o 'public RestTemplate restTemplate(final RestTemplateBuilder restTemplateBuilder)' {} \; > .bob/var.scrapedLine || true;
    find ./odp-token-service-api/src/main/java -regex '.*Application.java' -exec grep -o 'return restTemplateBuilder.build()' {} \; >> .bob/var.scrapedLine || true;
